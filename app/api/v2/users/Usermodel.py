...
class User(object):
	...
	def get_userByid(self,user_id):
		...
		cur.execute("SELECT * FROM tbl_users where status=1 and  id = %s",(str(user_id)))
		row = cur.fetchall()
		size =len(row)
		if  size > 0:
	...
	def login(email,password):
		...
		cur.execute("SELECT * FROM tbl_users where status=1 and  email = %s and password = %s" ,(email,password))
		row = cur.fetchall()
		size =len(row)
		if  size > 0:
