...
class User(object):
	...
	def update_user(self,user_id,user_data):
		...
		size =len(row)
		if  size > 0:
			cur.execute("UPDATE tbl_users set firstname = %s,lastname= %s,othername = %s,phoneNumber= %s, passportUrl= %s,email= %s  WHERE id = %s",(user_data['firstname'],user_data['lastname'],user_data['othername'],user_data['phoneNumber'],user_data['passportUrl'],user_data['email'],str(user_id)))
			return make_response(jsonify({'status':200, 'message': "user updated successfully"}), 200)
		return make_response(jsonify({'status':404, 'message': 'user not found'}),404)
